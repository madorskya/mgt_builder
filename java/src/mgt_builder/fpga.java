...
//create_clock -period 8.0 -name txclk0  [get_pins -filter {REF_PIN_NAME=~*TXOUTCLK} -of_objects [get_cells -hierarchical -filter {NAME =~ *quad_loop[0]*mgt_loop[3]*GTHE2_CHANNEL_inst*}]]
                    ...
                    if (m.rx_active)
                    {
                        ...
                        genclk_constr.add (String.format("create_clock -period %.3f -name rxclk%d_%d [get_pins -filter {REF_PIN_NAME=~*%s} -of_objects [get_cells -hierarchical -filter {NAME =~ *quad_loop[%d]*mgt_loop[%d]*GTHE2_CHANNEL_inst*}]]; # %s%d",
                                pcfg.rxoutclk_constr, x, y, qc.mgt_rxoutclk_name, qi, mi, m.rx_group_name, m.rx_group_index));
                    ...
                    if (m.tx_active)
                    {
                        ...
                        genclk_constr.add (String.format("create_clock -period %.3f -name txclk%d_%d [get_pins -filter {REF_PIN_NAME=~*%s} -of_objects [get_cells -hierarchical -filter {NAME =~ *quad_loop[%d]*mgt_loop[%d]*GTHE2_CHANNEL_inst*}]]; # %s%d",
                                pcfg.txoutclk_constr, x, y, qc.mgt_txoutclk_name, qi, mi, m.tx_group_name, m.tx_group_index));
