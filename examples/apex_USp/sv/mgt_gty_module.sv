// This code was automatically generated by FPGA MGT Builder
// Timestamp: 2020-11-06 10:51:37.69
// User: madorsky
// FPGA MGT builder homepage: <link>

`include "drp_interface.sv"
`include "mgt_gty_interfaces.sv"
module GTYE4_CHANNEL_tux
#(parameter [7:0] qind = 0, parameter [7:0] mind = 0)
(
	drp.in drp_gty_if,
	mgt_gty_tx.in mgt_gty_tx_if,
	mgt_gty_rx.in mgt_gty_rx_if,
	input         cplllockdetclk,
	input         dmonitorclk,
	input         drpclk,
	input         qpll0clk,
	input         qpll0refclk,
	input         qpll1clk,
	input         qpll1refclk,
	output        rxphaligndone,
	input         rxsyncallin,
	input         rxusrclk,
	input         rxusrclk2,
	input         txlatclk,
	input         txusrclk,
	input         txusrclk2
);
`include "xymap_gty.sv"
	wire [15:0] XY = mgtxy[qind][mind];
	reg  [31:0] reg_  [ 4: 0];
	wire [31:0] wire_ [ 6: 5];
    reg         int_reg_rdy;
    reg  [31:0] drpdo_reg;
    wire        drprdy_mgt;
    wire [31:0] drpdo_mgt;
    reg         drprdy_reg;
    assign drp_gty_if.drprdy = drprdy_reg;
    assign drp_gty_if.drpdo  = drpdo_reg;
    always @(posedge drpclk)
    begin
        drprdy_reg = 1'b0;
		drpdo_reg = 32'h0;
        if (drp_gty_if.int_reg == 1'b1)
        begin
            if (drp_gty_if.drpwe == 1'b1)
            begin
                case (drp_gty_if.drpaddr)

                    9'd0: reg_[0] = drp_gty_if.drpdi;
                    9'd1: reg_[1] = drp_gty_if.drpdi;
                    9'd2: reg_[2] = drp_gty_if.drpdi;
                    9'd3: reg_[3] = drp_gty_if.drpdi;
                    9'd4: reg_[4] = drp_gty_if.drpdi;
                endcase
                drprdy_reg = 1'b1;
            end
            else
            begin
                case (drp_gty_if.drpaddr)

                    9'd0: drpdo_reg = reg_[0];
                    9'd1: drpdo_reg = reg_[1];
                    9'd2: drpdo_reg = reg_[2];
                    9'd3: drpdo_reg = reg_[3];
                    9'd4: drpdo_reg = reg_[4];
                    9'd5: drpdo_reg = wire_[5];
                    9'd6: drpdo_reg = wire_[6];
                    9'h1ff: drpdo_reg = XY; // each MGT reports its XY
                endcase
                drprdy_reg = 1'b1;
            end
        end

        if (drprdy_mgt == 1'b1)
        begin
            drpdo_reg = drpdo_mgt;
            drprdy_reg = 1'b1;
        end
    end

	GTYE4_CHANNEL GTYE4_CHANNEL_inst
	(
		.CDRSTEPDIR                 (1'b0),
		.CDRSTEPSQ                  (1'b0),
		.CDRSTEPSX                  (1'b0),
		.CFGRESET                   ( reg_[0][ 0: 0]),
		.CPLLFBCLKLOST              (wire_[5][ 0: 0]),
		.CPLLLOCK                   (wire_[5][ 1: 1]),
		.CPLLLOCKDETCLK             (cplllockdetclk            ),
		.CPLLLOCKEN                 (1'b1),
		.CPLLPD                     ( reg_[0][ 1: 1]),
		.CPLLREFCLKLOST             (wire_[5][ 2: 2]),
		.CPLLREFCLKSEL              (3'b1),
		.CPLLRESET                  (1'b0),
		.DMONFIFORESET              ( reg_[0][ 2: 2]),
		.DMONITORCLK                (dmonitorclk               ),
		.DMONITOROUT                (wire_[5][18: 3]),
		.DMONITOROUTCLK             (wire_[5][19:19]),
		.DRPADDR                    (drp_gty_if.drpaddr        ),
		.DRPCLK                     (drpclk                    ),
		.DRPDI                      (drp_gty_if.drpdi [15:0]),
		.DRPDO                      (drpdo_mgt [15:0]),
		.DRPEN                      (drp_gty_if.drpen          ),
		.DRPRDY                     (drprdy_mgt),
		.DRPRST                     ( reg_[0][ 3: 3]),
		.DRPWE                      (drp_gty_if.drpwe          ),
		.EYESCANDATAERROR           (wire_[5][20:20]),
		.EYESCANRESET               ( reg_[0][ 4: 4]),
		.EYESCANTRIGGER             ( reg_[0][ 5: 5]),
		.FREQOS                     (1'b0),
		.GTGREFCLK                  (1'b0),
		.GTNORTHREFCLK0             (1'b0),
		.GTNORTHREFCLK1             (1'b0),
		.GTPOWERGOOD                (wire_[5][21:21]),
		.GTREFCLK0                  (1'b0),
		.GTREFCLK1                  (1'b0),
		.GTREFCLKMONITOR            (),
		.GTRESETSEL                 ( reg_[0][ 6: 6]),
		.GTRXRESET                  ( reg_[0][ 7: 7]),
		.GTRXRESETSEL               ( reg_[0][ 8: 8]),
		.GTSOUTHREFCLK0             (1'b0),
		.GTSOUTHREFCLK1             (1'b0),
		.GTTXRESET                  ( reg_[0][ 9: 9]),
		.GTTXRESETSEL               ( reg_[0][10:10]),
		.GTYRXN                     (mgt_gty_rx_if.gtyrxn      ),
		.GTYRXP                     (mgt_gty_rx_if.gtyrxp      ),
		.INCPCTRL                   (1'b0),
		.LOOPBACK                   ( reg_[0][13:11]),
		.MGTYTXN                    (mgt_gty_tx_if.mgtytxn     ),
		.MGTYTXP                    (mgt_gty_tx_if.mgtytxp     ),
		.PCSRSVDOUT                 (),
		.PHYSTATUS                  (),
		.QPLL0CLK                   (qpll0clk                  ),
		.QPLL0REFCLK                (qpll0refclk               ),
		.QPLL1CLK                   (qpll1clk                  ),
		.QPLL1REFCLK                (qpll1refclk               ),
		.RESETEXCEPTION             (),
		.RESETOVRD                  (1'b0),
		.RSOSINTDONE                (),
		.RX8B10BEN                  ( reg_[0][14:14]),
		.RXAFECFOKEN                (1'b1),
		.RXBUFRESET                 ( reg_[0][15:15]),
		.RXBUFSTATUS                (wire_[5][24:22]),
		.RXBYTEISALIGNED            (wire_[5][25:25]),
		.RXBYTEREALIGN              (),
		.RXCDRFREQRESET             ( reg_[0][16:16]),
		.RXCDRHOLD                  ( reg_[0][17:17]),
		.RXCDRLOCK                  (),
		.RXCDROVRDEN                (1'b0),
		.RXCDRPHDONE                (),
		.RXCDRRESET                 (1'b0),
		.RXCHANBONDSEQ              (),
		.RXCHANISALIGNED            (),
		.RXCHANREALIGN              (),
		.RXCHBONDEN                 (1'b0),
		.RXCHBONDI                  (5'b0),
		.RXCHBONDLEVEL              (3'b0),
		.RXCHBONDMASTER             (1'b0),
		.RXCHBONDO                  (),
		.RXCHBONDSLAVE              (1'b0),
		.RXCKCALDONE                (),
		.RXCKCALRESET               (1'b0),
		.RXCKCALSTART               (7'b0),
		.RXCLKCORCNT                (),
		.RXCOMINITDET               (),
		.RXCOMMADET                 (),
		.RXCOMMADETEN               ( reg_[0][18:18]),
		.RXCOMSASDET                (),
		.RXCOMWAKEDET               (),
		.RXCTRL0                    (mgt_gty_rx_if.rxctrl0     ),
		.RXCTRL1                    (mgt_gty_rx_if.rxctrl1     ),
		.RXCTRL2                    (mgt_gty_rx_if.rxctrl2     ),
		.RXCTRL3                    (mgt_gty_rx_if.rxctrl3     ),
		.RXDATA                     (mgt_gty_rx_if.rxdata      ),
		.RXDATAEXTENDRSVD           (),
		.RXDATAVALID                (mgt_gty_rx_if.rxdatavalid ),
		.RXDFEAGCHOLD               (1'b0),
		.RXDFEAGCOVRDEN             (1'b0),
		.RXDFECFOKFCNUM             (4'b1101),
		.RXDFECFOKFEN               (1'b0),
		.RXDFECFOKFPULSE            (1'b0),
		.RXDFECFOKHOLD              (1'b0),
		.RXDFECFOKOVREN             (1'b0),
		.RXDFEKHHOLD                (1'b0),
		.RXDFEKHOVRDEN              (1'b0),
		.RXDFELFHOLD                (1'b0),
		.RXDFELFOVRDEN              (1'b0),
		.RXDFELPMRESET              ( reg_[0][19:19]),
		.RXDFETAP10HOLD             (1'b0),
		.RXDFETAP10OVRDEN           (1'b0),
		.RXDFETAP11HOLD             (1'b0),
		.RXDFETAP11OVRDEN           (1'b0),
		.RXDFETAP12HOLD             (1'b0),
		.RXDFETAP12OVRDEN           (1'b0),
		.RXDFETAP13HOLD             (1'b0),
		.RXDFETAP13OVRDEN           (1'b0),
		.RXDFETAP14HOLD             (1'b0),
		.RXDFETAP14OVRDEN           (1'b0),
		.RXDFETAP15HOLD             (1'b0),
		.RXDFETAP15OVRDEN           (1'b0),
		.RXDFETAP2HOLD              (1'b0),
		.RXDFETAP2OVRDEN            (1'b0),
		.RXDFETAP3HOLD              (1'b0),
		.RXDFETAP3OVRDEN            (1'b0),
		.RXDFETAP4HOLD              (1'b0),
		.RXDFETAP4OVRDEN            (1'b0),
		.RXDFETAP5HOLD              (1'b0),
		.RXDFETAP5OVRDEN            (1'b0),
		.RXDFETAP6HOLD              (1'b0),
		.RXDFETAP6OVRDEN            (1'b0),
		.RXDFETAP7HOLD              (1'b0),
		.RXDFETAP7OVRDEN            (1'b0),
		.RXDFETAP8HOLD              (1'b0),
		.RXDFETAP8OVRDEN            (1'b0),
		.RXDFETAP9HOLD              (1'b0),
		.RXDFETAP9OVRDEN            (1'b0),
		.RXDFEUTHOLD                (1'b0),
		.RXDFEUTOVRDEN              (1'b0),
		.RXDFEVPHOLD                (1'b0),
		.RXDFEVPOVRDEN              (1'b0),
		.RXDFEXYDEN                 (1'b1),
		.RXDLYBYPASS                ( reg_[0][20:20]),
		.RXDLYEN                    ( reg_[0][21:21]),
		.RXDLYOVRDEN                ( reg_[0][22:22]),
		.RXDLYSRESET                ( reg_[0][23:23]),
		.RXDLYSRESETDONE            (wire_[5][26:26]),
		.RXELECIDLE                 (),
		.RXELECIDLEMODE             ( reg_[0][25:24]),
		.RXGEARBOXSLIP              (mgt_gty_rx_if.rxgearboxslip),
		.RXHEADER                   (mgt_gty_rx_if.rxheader    ),
		.RXHEADERVALID              (mgt_gty_rx_if.rxheadervalid),
		.RXLATCLK                   (mgt_gty_rx_if.rxlatclk    ),
		.RXLPMEN                    ( reg_[0][26:26]),
		.RXLPMGCHOLD                (1'b0),
		.RXLPMGCOVRDEN              (1'b0),
		.RXLPMHFHOLD                (1'b0),
		.RXLPMHFOVRDEN              (1'b0),
		.RXLPMLFHOLD                (1'b0),
		.RXLPMLFKLOVRDEN            (1'b0),
		.RXLPMOSHOLD                (1'b0),
		.RXLPMOSOVRDEN              (1'b0),
		.RXMCOMMAALIGNEN            ( reg_[0][27:27]),
		.RXMONITOROUT               (),
		.RXMONITORSEL               (2'b00),
		.RXOOBRESET                 (1'b0),
		.RXOSCALRESET               ( reg_[0][28:28]),
		.RXOSHOLD                   (1'b0),
		.RXOSINTDONE                (),
		.RXOSINTSTARTED             (),
		.RXOSINTSTROBEDONE          (),
		.RXOSINTSTROBESTARTED       (1'b0),
		.RXOSOVRDEN                 (1'b0),
		.RXOUTCLK                   (mgt_gty_rx_if.rxoutclk    ),
		.RXOUTCLKFABRIC             (),
		.RXOUTCLKPCS                (),
		.RXOUTCLKSEL                ( reg_[0][31:29]),
		.RXPCOMMAALIGNEN            ( reg_[1][ 0: 0]),
		.RXPCSRESET                 ( reg_[1][ 1: 1]),
		.RXPD                       ( reg_[1][ 3: 2]),
		.RXPHALIGN                  ( reg_[1][ 4: 4]),
		.RXPHALIGNDONE              (rxphaligndone             ),
		.RXPHALIGNEN                ( reg_[1][ 5: 5]),
		.RXPHALIGNERR               (wire_[5][27:27]),
		.RXPHDLYPD                  (1'b0),
		.RXPHDLYRESET               ( reg_[1][ 6: 6]),
		.RXPLLCLKSEL                ( reg_[1][ 8: 7]),
		.RXPMARESET                 ( reg_[1][ 9: 9]),
		.RXPMARESETDONE             (wire_[5][28:28]),
		.RXPOLARITY                 ( reg_[1][10:10]),
		.RXPRBSCNTRESET             ( reg_[1][11:11]),
		.RXPRBSERR                  (wire_[5][29:29]),
		.RXPRBSLOCKED               (wire_[5][30:30]),
		.RXPRBSSEL                  ( reg_[1][15:12]),
		.RXPRGDIVRESETDONE          (wire_[5][31:31]),
		.RXPROGDIVRESET             ( reg_[1][16:16]),
		.RXRATE                     ( reg_[1][19:17]),
		.RXRATEDONE                 (),
		.RXRATEMODE                 ( reg_[1][20:20]),
		.RXRESETDONE                (wire_[6][ 0: 0]),
		.RXSLIDE                    (mgt_gty_rx_if.rxslide     ),
		.RXSLIPOUTCLK               ( reg_[1][21:21]),
		.RXSLIPPMA                  ( reg_[1][22:22]),
		.RXSTARTOFSEQ               (mgt_gty_rx_if.rxstartofseq),
		.RXSTATUS                   (),
		.RXSYNCALLIN                (rxsyncallin               ),
		.RXSYNCDONE                 (wire_[6][ 1: 1]),
		.RXSYNCIN                   ( reg_[1][23:23]),
		.RXSYNCMODE                 ( reg_[1][24:24]),
		.RXSYNCOUT                  (wire_[6][ 2: 2]),
		.RXSYSCLKSEL                ( reg_[1][26:25]),
		.RXTERMINATION              (1'b0),
		.RXUSERRDY                  ( reg_[1][27:27]),
		.RXUSRCLK                   (rxusrclk                  ),
		.RXUSRCLK2                  (rxusrclk2                 ),
		.TX8B10BBYPASS              ( reg_[2][ 7: 0]),
		.TX8B10BEN                  ( reg_[2][ 8: 8]),
		.TXBUFSTATUS                (wire_[6][ 4: 3]),
		.TXCOMFINISH                (),
		.TXCOMINIT                  (1'b0),
		.TXCOMSAS                   (1'b0),
		.TXCOMWAKE                  (1'b0),
		.TXCTRL0                    (mgt_gty_tx_if.txctrl0     ),
		.TXCTRL1                    (mgt_gty_tx_if.txctrl1     ),
		.TXCTRL2                    (mgt_gty_tx_if.txctrl2     ),
		.TXDATA                     (mgt_gty_tx_if.txdata      ),
		.TXDATAEXTENDRSVD           (1'b0),
		.TXDCCDONE                  (),
		.TXDCCFORCESTART            (),
		.TXDCCRESET                 (),
		.TXDEEMPH                   ( reg_[2][10: 9]),
		.TXDETECTRX                 (1'b0),
		.TXDIFFCTRL                 ( reg_[2][15:11]),
		.TXDLYBYPASS                ( reg_[2][16:16]),
		.TXDLYEN                    ( reg_[2][17:17]),
		.TXDLYHOLD                  (1'b0),
		.TXDLYOVRDEN                ( reg_[2][18:18]),
		.TXDLYSRESET                ( reg_[2][19:19]),
		.TXDLYSRESETDONE            (wire_[6][ 5: 5]),
		.TXDLYUPDOWN                (1'b0),
		.TXELECIDLE                 ( reg_[2][20:20]),
		.TXHEADER                   (mgt_gty_tx_if.txheader    ),
		.TXINHIBIT                  ( reg_[2][21:21]),
		.TXLATCLK                   (txlatclk                  ),
		.TXMAINCURSOR               ( reg_[2][28:22]),
		.TXMARGIN                   ( reg_[2][31:29]),
		.TXOUTCLK                   (mgt_gty_tx_if.txoutclk    ),
		.TXOUTCLKFABRIC             (),
		.TXOUTCLKPCS                (),
		.TXOUTCLKSEL                ( reg_[3][ 2: 0]),
		.TXPCSRESET                 ( reg_[3][ 3: 3]),
		.TXPD                       ( reg_[3][ 5: 4]),
		.TXPDELECIDLEMODE           (1'b0),
		.TXPHALIGN                  ( reg_[3][ 6: 6]),
		.TXPHALIGNDONE              (wire_[6][ 6: 6]),
		.TXPHALIGNEN                ( reg_[3][ 7: 7]),
		.TXPHDLYPD                  (1'b0),
		.TXPHDLYRESET               ( reg_[3][ 8: 8]),
		.TXPHDLYTSTCLK              (1'b0),
		.TXPHINIT                   (1'b0),
		.TXPHINITDONE               (wire_[6][ 7: 7]),
		.TXPHOVRDEN                 ( reg_[3][ 9: 9]),
		.TXPIPPMEN                  ( reg_[3][10:10]),
		.TXPIPPMOVRDEN              ( reg_[3][11:11]),
		.TXPIPPMPD                  ( reg_[3][12:12]),
		.TXPIPPMSEL                 ( reg_[3][13:13]),
		.TXPIPPMSTEPSIZE            ( reg_[3][18:14]),
		.TXPISOPD                   (1'b0),
		.TXPLLCLKSEL                ( reg_[3][20:19]),
		.TXPMARESET                 ( reg_[3][21:21]),
		.TXPMARESETDONE             (wire_[6][ 8: 8]),
		.TXPOLARITY                 ( reg_[3][22:22]),
		.TXPOSTCURSOR               ( reg_[3][27:23]),
		.TXPRBSFORCEERR             ( reg_[3][28:28]),
		.TXPRBSSEL                  ( reg_[4][ 3: 0]),
		.TXPRECURSOR                ( reg_[4][ 8: 4]),
		.TXPRGDIVRESETDONE          (wire_[6][ 9: 9]),
		.TXPROGDIVRESET             ( reg_[4][ 9: 9]),
		.TXRATE                     ( reg_[4][12:10]),
		.TXRATEDONE                 (wire_[6][10:10]),
		.TXRATEMODE                 ( reg_[4][13:13]),
		.TXRESETDONE                (wire_[6][11:11]),
		.TXSEQUENCE                 (mgt_gty_tx_if.txsequence  ),
		.TXSWING                    ( reg_[4][14:14]),
		.TXSYNCALLIN                ( reg_[4][15:15]),
		.TXSYNCDONE                 (wire_[6][12:12]),
		.TXSYNCIN                   ( reg_[4][16:16]),
		.TXSYNCMODE                 ( reg_[4][17:17]),
		.TXSYNCOUT                  (wire_[6][13:13]),
		.TXSYSCLKSEL                ( reg_[4][19:18]),
		.TXUSERRDY                  ( reg_[4][20:20]),
		.TXUSRCLK                   (txusrclk                  ),
		.TXUSRCLK2                  (txusrclk2                 )
	);
endmodule
