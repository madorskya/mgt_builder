...
void eyescan_all (string cmd)
{
  ...
  vector <string> argv;
	...
	int scale  = strtol (argv[2].c_str(), NULL, 10);
	int mode   = strtol (argv[3].c_str(), NULL, 10);
...
node_record nr[] =
{
	...
	{0, "scanall",           "gth | gty",          NULL,           NULL},
	{1,     "gth",   		     "scale(6-15)",          NULL,           NULL},
	{1,     "gty",     		   "scale(6-15)",          NULL,           NULL},
	{2,     "([0-9]+)",      "mode:normal-2d(0) | bathtub(1)",  NULL,       NULL},
	{3,     "([0-1])",       "<Enter>",         eyescan_all,        NULL},
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        ...
        if (true) // disabling serialization, does not work reliably
		{
            ...
            chip.read_top_config (argv[1]); // quick. top configuration file
                gettimeofday(&t[1], NULL);
            chip.read_mgt_config (); // quick. read MGT configuration common parameters
                gettimeofday(&t[2], NULL);
            chip.read_mgt_list   (); // slowest. fpga link base addresses. Creates all drp_unit objects, reads DRP, port maps into them
                gettimeofday(&t[3], NULL);
            chip.read_links      (); // quick. link config file, same as what java script reads. Makes links active, assigns protocol name
                gettimeofday(&t[4], NULL);
            chip.read_params     (); // slow. DRP registers' and port settings for each link, according to protocol. File names are created automatically
                gettimeofday(&t[5], NULL);
            chip.read_inversions (); // quick. read inversion flags for each link
